...
// by using the methods of the [DataStoreBuilder] returned by [DataStore]:
//
//     config := ld.Config{
...
//     }
//
// Note that CacheSeconds() is not a method of [DataStoreBuilder], but rather a method of
// ldcomponents.PersistentDataStore(), because the caching behavior is provided by the SDK for
// all database integrations.
//
// For advanced customization of the underlying Redigo client, use [DataStoreBuilder] methods such
// as [DataStoreBuilder.DialOptions] and [DataStoreBuilder.Pool]. Note that some Redis client features
